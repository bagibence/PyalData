...
def trial_average(trial_data, condition, ref_field=None):
    ...
    return pd.DataFrame.from_dict(
        {a: trial_average(b, None) for (a, b) in trial_data.groupby(groups)}, orient="index"
...
def keep_common_trials(
    df_a: pd.DataFrame, df_b: pd.DataFrame, join_field: str = "trial_id"
):
    """
    ...
    """
    if df_a[join_field].duplicated().sum() > 0:
        warnings.warn(
            f"df_a contains duplicate values for {join_field}. Consider dropping them before using this function."
        )
    if df_b[join_field].duplicated().sum() > 0:
        warnings.warn(
            f"df_b contains duplicate values for {join_field}. Consider dropping them before using this function."
        )
    ...
    subset_a = df_a.query(f"{join_field} in @common_ids")
    subset_b = df_b.query(f"{join_field} in @common_ids")
